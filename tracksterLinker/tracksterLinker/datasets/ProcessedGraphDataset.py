...
class ProcessedGraphDataset(Dataset):
    ...
    def __init__(
        ...
        **_,
        ...
        self.node_scaler = node_scaler if node_scaler is not None else self._load_scaler("node_scaler.pt")
        self.edge_scaler = edge_scaler if edge_scaler is not None else self._load_scaler("edge_scaler.pt")
    ...
    def get(self, idx):
        return torch.load(osp.join(self.processed_dir, f"data_{idx}.pt"), weights_only=False, map_location=torch.device("cpu"))
