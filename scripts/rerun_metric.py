...
def build_model(architecture, input_dim, edge_dim, device):
    ...
    if architecture == "punet":
        return PUNet(edge_hidden_dim=16, hidden_dim=32, num_heads=4, **kwargs).to(device)
    if architecture == "gnn":
...
def load_model(checkpoint_path, sample, input_dim, device, args):
    checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False, mmap='cpu')
    ...
    architecture = checkpoint.get("architecture") or metadata.get("architecture") or config.get("architecture", "punet")
...
def stability_records(args, dataset, dataset_cls, focal_model, contrastive_model, device):
    ...
    for graph_idx in range(n_graphs):
        ...
        contrastive_flip_rate = contrastive_flips
