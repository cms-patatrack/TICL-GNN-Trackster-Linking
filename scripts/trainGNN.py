...
load_weights = True
model_name = "model_2025-09-22_epoch_4_dict"
...
early_stopping = EarlyStopping(patience=20, delta=0.05)
...
for epoch in range(start_epoch, start_epoch+epochs):
    ...
    loss = train(model, optimizer, train_dl, epoch+1, scores=True, device=device, loss_obj=loss_obj)
    train_loss_hist.append(loss)

    val_loss, cross_edges, signal_edges, pu_edges = test(model, test_dl, epoch+1, loss_obj=loss_obj, scores=True, device=device, weighted="raw_energy")
    ...
    if ((epoch+1) % 10 == 0):
        ...
        val_loss, pred, y, weight, PU_info = validate(model, test_dl, epoch+1, loss_obj=loss_obj, scores=True, device=device, weighted="raw_energy")
