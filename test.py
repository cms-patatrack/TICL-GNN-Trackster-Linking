...
def plot_validation_results(pred, y, save=True, output_folder=None, file_suffix=None):
    ...
    plot_roc_curve(pred, y, ax[0, 1])
    plot_confusion_matrix(pred, y, ax[1, :])

    best_threshold = get_best_threshold(TNR, TPR, thresholds)
    plot_prediction_distribution(pred, y, ax[2:5, :], thres=best_threshold)
...
def plot_edge_distribution(pred, y, axes):
    ...
    axes[0].set_title("True and False edge prediction distribution", fontsize=15)
    ...
    axes[1].legend()
    axes[1].set_title("Edge prediction distribution", fontsize=15)
...
def print_acc_scores(pred, y, thres=0.65):
    ...
    prec_w, rec_w, fscore_w, _ = precision_recall_fscore_support(y, pred, sample_weight=sample_weight)
    ...
    pos_lr, neg_lr = class_likelihood_ratios(y, pred, raise_warning=False)
    print(f"positive_likelihood_ratio: {pos_lr}, negative_likelihood_ratio: {neg_lr}")


def plot_roc_curve(pred, y, ax):
...
def plot_prediction_distribution(pred, y, axes, thr=0.65):
    ...
    axes[1, 0].set_title('True Edge Labels')

    axes[1, 1].hist(y, bins=30)
    axes[1, 1].set_title('True Edge Labels Log')
    axes[1, 1].set_yscale('log')
    # ------------------------

    # Thresholded labels in linear and log scales
    thresholded = (pred > thr).astype(int)
    axes[2, 0].set_title(f'Predicted Edge Labels for thr: {thr}')
    axes[2, 0].hist(thresholded, bins=30)

    axes[2, 1].set_title(f'Predicted Edge Labels Log for thr: {thr}')
